#!/bin/bash
# Generate GitHub issues from Stuur user stories
...
# Function to create an issue file
create_issue() {
    local id="$1"
    local epic="$2"
    local title="$3"
    ...
    local filename="$ISSUES_DIR/${id}.md"
...
# Create epic overview files first
create_epic_overview() {
    local epic_id="$1"
    local epic_name="$2"
    local epic_desc="$3"
    
    local filename="$EPICS_DIR/${epic_id}.md"
...
EOF
    ...
    echo "Created epic: $filename"
}

# Create epic overviews
create_epic_overview "EP01" "Gateway & Telegram" "Telegram bot gateway with error handling and fast responses"
create_epic_overview "EP02" "SurrealDB & Data Layer" "Database layer with persistence, typed access, and backups"
create_epic_overview "EP03" "Identity Core" "User profiling, learning, memory, and personalization"
create_epic_overview "EP04" "Intent Router" "Semantic routing for natural language classification"
create_epic_overview "EP05" "Ollama / Local LLM (Tier 2)" "Local LLM integration for cost-effective processing"
create_epic_overview "EP06" "Claude Integration (Tier 3)" "Cloud LLM integration for complex conversations"
create_epic_overview "EP07" "Smart Reminders" "Natural language reminders with smart defaults"
create_epic_overview "EP08" "Google Calendar" "Calendar integration for event management"
create_epic_overview "EP09" "Expense Tracker" "Spending tracking with auto-categorization"
create_epic_overview "EP10" "Personal CRM" "Contact management and relationship tracking"
create_epic_overview "EP11" "Goal Tracker" "Goal setting and progress tracking"
create_epic_overview "EP12" "Hardening & Open Source" "Production readiness and open source setup"
create_epic_overview "EP13" "Growth & Monetization" "Scaling, monetization, and multi-channel support"

# Now create all user stories
